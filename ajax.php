...
function getSlides($data) {
   ...
   if(isset($data['idplato'])){
       if($data['idplato']>0){
            $randon = " letra";
            $query.= "select 'A' as letra, S.*, l.name,SP.idsection from seccion_lima S  inner join locations l on l.id=S.departamento  inner join seccions_platos SP on SP.idplato = S.id where SP.idsection = ".(int)$data['dishes']." and l.name = '".$data['city']."' and S.id= '".$data['idplato']."' UNION ";
